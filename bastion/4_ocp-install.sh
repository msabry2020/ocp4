...
echo "initialize install-config variables\n"
...
echo "Create the install directory\n"
rm -rf $INSTALL_DIR
mkdir $INSTALL_DIR

echo "Copy the install-config.yaml file to the install directory\n"
cp install-config.yaml $INSTALL_DIR

echo "Change directory to the install directory\n"
cd $INSTALL_DIR

echo "Edit the the install-config file\n"
...
echo "Backup install-config file\n"
cp install-config.yaml $INSTALL_HOME

echo "Create the manifests\n"
openshift-install create manifests --dir=.

echo "Remove the master and worker machineset manifests\n"
rm -f openshift/99_openshift-cluster-api_master-machines-*.yaml openshift/99_openshift-cluster-api_worker-machineset-*.yaml

echo "Edit the cluster-scheduler-02-config.yml file to set mastersSchedulable to false\n"
sed -i 's/mastersSchedulable: true/mastersSchedulable: false/g' manifests/cluster-scheduler-02-config.yml

echo "Enable verbose output for debugging\n"
set -x

echo "Create the ignition configs\n"
openshift-install create ignition-configs --dir=.
set +x

echo "Change the permissions of the ignition config files\n"
chmod +r *.ign

echo "Copy the ignition config files to the utility node\n"
rsync -av *.ign utility.$CLUSTER_NAME.$BASE_DOMAIN:/var/www/html/openshift4/ignitions/

echo "Create base64-encoded ignition files\n"
